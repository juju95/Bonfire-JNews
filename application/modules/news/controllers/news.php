...
class News extends Front_Controller {
    ...
    public function get_article_list($limit=-1, $offset=0)
	{
			...
			$articles = $this->news_model->where('deleted', 0)->get_articles(true, $limit, $offset, true);
			foreach($articles AS &$article)
			{
				$article->titleurl = url_title($article->title, '-', TRUE);
			}
			//print_r($articles);
			/*
    ...
    public function get_articles($limit=-1, $offset=0)
	{
		...
		if (is_array($articles) && count($articles))
		{
			...
			foreach ($articles as $article)
			{
				...
				$article->titleurl = url_title($article->title);
